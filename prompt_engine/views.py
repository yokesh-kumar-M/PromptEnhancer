...
def enhance_prompt(request):
    ...
    try:
        ...
        EnhancementLog.objects.create(
            ...
            enhanced_char_count=len(response.text),
        )

        return JsonResponse({'enhanced': response.text})
        ...
        try:
            send_mail(
                ...
                message=(
                    ...
                    f'Approve this request in the admin panel:\n'
...
def dashboard_view(request):
    ...
    if profile.invite_code:
        logs = EnhancementLog.objects.filter(
            Q(user=request.user) | Q(invite_code=profile.invite_code)
        ).distinct()
    else:
        logs = EnhancementLog.objects.filter(user=request.user)
    ...
    for log in logs.values('action', 'provider'):
    ...
    recent = logs.select_related('invite_code')[:15]

    return render(request, 'dashboard.html', {
        'profile': profile,
        'stats': {'total': total, 'today': today_count, 'week': week_count},
